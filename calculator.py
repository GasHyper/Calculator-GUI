...
def buttonClick(num):  # function for clicking
    var = e.get(
    )  # varibale to store the current input in the screen
    e.delete(0, tk.END)  # clearing the screen from index 0 to END
    e.insert(0, var + num)  # inserting the incoming number input
...
def buttonGet(operator):  # function for storing the first input and printing '+, -, /, *'
    global num1, math  # global variable num1 and math to use in function buttonEqual()
    num1 = e.get()  # getting first number
    math = operator  # operator variable is the type of operation being performed
...
def buttonEqual():  # function for printing the sum
    ...
    if math == '+':  # Addition
        ...
        try:
            ...
            # in case there is a zero in the denominator, answer is ZeroError
            e.insert(0, 'ZeroError')
